...
function getUniqueDeliveryDates(items) {
  const deliveryDates = items.map((item) => calculateDeliveryDateForItem(item));
...
function generatePDFHTML(items, cartDate, deliveryDate = null) {
  ...
  if (deliveryDate) {
    html += `<h3>Доставка на: ${deliveryDate}</h3>`;
  }
  ...
  items.forEach((item, index) => {
    html += `
      <tr style="height: ${imageCellWidth}px; background-color: white;">
        <td>${index + 1}</td>
        <td style="width: ${imageCellWidth}px; height: ${imageCellWidth}px; text-align: center; vertical-align: middle; background-color: white;">
          <img src="${
            item["Изображение"]
